...
def test_frame_record_uses_previous_tiles_not_next():
    ...
    # were emitted on frame 4's probe (= prev_tiles). The record must carry
    # prev_tiles (categorised dicts), NOT the tiles just computed for frame 6.
    prev_tiles = [{"x": 0.1, "y": 0.1, "w": 0.1, "h": 0.1, "category": "multi-scale"}]
    next_tiles = [{"x": 0.8, "y": 0.8, "w": 0.1, "h": 0.1, "category": "dynamic"}]
    dets = [{"label": "vehicle", "confidence": 0.9, "bbox": [0.12, 0.12, 0.03, 0.02]}]
    rec = build_frame_record(frame=5, detections=dets, applied_tiles=prev_tiles)
    assert rec["frame"] == 5
    assert rec["detections"] == dets
    assert rec["tiles"] == prev_tiles
    assert rec["tiles"] != next_tiles
