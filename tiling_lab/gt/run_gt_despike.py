...
windowed-median height back up to that median. --anchor top (default) holds the
top edge so the box grows downward to the feet (legs cut); --anchor bottom holds
the bottom edge so the box grows upward over the head (head cut). Rewrites:
...
def main():
    ...
    ap.add_argument("--anchor", choices=["top", "bottom"], default="top",
                    help="held vertical edge: 'top' grows downward (feet cut, default), "
    ...
    corrections["despike_track_heights"] = {"track_ids": ids, "min_ratio": args.min_ratio,
