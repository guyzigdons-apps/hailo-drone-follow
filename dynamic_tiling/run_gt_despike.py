"""CLI: restore detector-truncated bbox heights ("only top of person") glitches.
    ...
    python -m dynamic_tiling.run_gt_despike --outdir <gt_verify dir> \
...
windowed-median height back up to that median, anchored at the top edge (so the
box grows downward to the feet). Rewrites:
...
def main():
    ...
    corrections["despike_track_heights"] = {"track_ids": ids, "min_ratio": args.min_ratio,
                                             "window": args.window, "source": args.source}
    corr_path.write_text(json.dumps(corrections, indent=2))
    print(f"despiked heights (min_ratio={args.min_ratio}, window={args.window}) for tracks {ids}")
