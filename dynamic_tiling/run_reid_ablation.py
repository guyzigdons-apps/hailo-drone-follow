...
Usage:
    source setup_env.sh
    python -m dynamic_tiling.run_reid_ablation                  # full ablation
    python -m dynamic_tiling.run_reid_ablation --gallery-sub-ablation
    python -m dynamic_tiling.run_reid_ablation --render-only     # MD from JSONs
"""
...
from .reid_policy import (POLICIES, GenerousPolicy, ProdPolicy, AmbiguityPolicy,
...
def render_report(outdir: Path) -> str:
    ...
    lines.append("python -m dynamic_tiling.run_reid_ablation "
                 f"--clips {DEFAULT_CLIPS} --arms {DEFAULT_ARMS} "
                 f"--budget {int(DEFAULT_BUDGET)}")
    lines.append("python -m dynamic_tiling.run_reid_ablation --gallery-sub-ablation")
    lines.append("python -m dynamic_tiling.run_reid_ablation --render-only")
    lines.append("```")
    lines.append("")
    if main:
