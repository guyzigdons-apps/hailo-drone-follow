...
class TestGestureLateral:
    ...
    def test_hand_right_of_center_positive_lateral(self):
        """Hand right of frame center -> positive lateral (strafe right)."""
        g = _gesture(hand=_hand(cx=0.7, is_open=True))
        lateral = compute_gesture_lateral(g, _config())
        assert lateral > 0.0

    def test_hand_left_of_center_negative_lateral(self):
        """Hand left of frame center -> negative lateral (strafe left)."""
        g = _gesture(hand=_hand(cx=0.3, is_open=True))
        lateral = compute_gesture_lateral(g, _config())
        assert lateral < 0.0

    def test_hand_centered_dead_zone(self):
        """Hand near frame center (within dead zone) -> zero lateral."""
        g = _gesture(hand=_hand(cx=0.51, is_open=True))
    ...
    def test_clamped_to_max(self):
        ...
        g = _gesture(hand=_hand(cx=0.9, is_open=True))
        cfg = _config(gesture_max_lateral=1.5)
        lateral = compute_gesture_lateral(g, cfg)
        assert abs(lateral) <= cfg.gesture_max_lateral + 0.01

    def test_proportional_to_offset(self):
        """Larger offset from center -> larger lateral magnitude."""
        g_small = _gesture(hand=_hand(cx=0.55, is_open=True))
        g_large = _gesture(hand=_hand(cx=0.65, is_open=True))
