...
#if defined(HAVE_GSTHAILOMETA)
// Restore cached detections from the writer's dets_json into the buffer's
// HailoROI, so a downstream tap (or hailofilter-equivalent bypass) reads
// the SAME per-tile detection stream the live pass produced — this is what
// the bit-exact replay gate (Task 6) compares.
//
// The payload is machine-generated by the writer
// (gst_hailocachewriter.cpp:read_tile_dets_json_) in the canonical shape
//   [{"cls":<int>,"score":<f>,"x":<f>,"y":<f>,"w":<f>,"h":<f>}, ...]
// with tile-local normalized coords. The field names are the shared
// constants in cache_keys.hpp (kDetField*) so writer-emit and reader-parse
// can't drift apart. We parse it with a tiny field scanner (no JSON lib
// dependency) keyed by the `"<key>":` tokens, so field order is irrelevant
// and trailing precision is preserved exactly (strtod on the %.9g text the
// writer wrote).
//
// The substring-search parser relies on two invariants of that schema: no
// field name is a substring of another (so `find("<name>":")` can't match
// the wrong key), and there are no string-typed values (whose contents could
// otherwise contain a spurious `"<name>":` token). Both hold for the
// numbers-only {cls,score,x,y,w,h} shape above.
//
// The reader is the on-disk trust boundary: a corrupted, hand-edited, or
// foreign cache row must degrade gracefully (skip the bad detection) rather
// than abort the streaming thread. See restore_cached_dets_to_roi below.
static bool
extract_number_field(const std::string& obj, const char* key, double* out)
{
...
static void
restore_cached_dets_to_roi(GstBuffer* buf, const std::string& dets_json)
{
    if (dets_json.empty() || dets_json == "[]") return;
    HailoROIPtr roi = get_hailo_main_roi(buf, /*create_if_missing=*/false);
    if (!roi) return;

    // Walk top-level {...} objects. The payload is flat (no nested braces),
    // so a brace-depth scan splits the array cleanly.
    size_t i = 0;
    const size_t n = dets_json.size();
    while (i < n) {
        if (dets_json[i] != '{') { ++i; continue; }
        size_t j = dets_json.find('}', i);
        if (j == std::string::npos) break;
        std::string obj = dets_json.substr(i, j - i + 1);
        double cls = 0, score = 0, x = 0, y = 0, w = 0, h = 0;
        bool ok = extract_number_field(obj, hailo_cache::kDetFieldCls,   &cls)
                  && extract_number_field(obj, hailo_cache::kDetFieldScore, &score)
                  && extract_number_field(obj, hailo_cache::kDetFieldX,     &x)
                  && extract_number_field(obj, hailo_cache::kDetFieldY,     &y)
                  && extract_number_field(obj, hailo_cache::kDetFieldW,     &w)
                  && extract_number_field(obj, hailo_cache::kDetFieldH,     &h);
        if (ok) {
            // Trust-boundary validation: the TAPPAS HailoDetection ctor calls
            // assure_normal(confidence) which THROWS std::invalid_argument
            // for score outside [0,1]. A corrupted / hand-edited / foreign
            // cache row could carry such a score and would otherwise abort
            // the streaming thread. Skip any detection with a NaN or
            // out-of-range score (rate-limited one-shot warning), and wrap
            // construction in a catch as defense-in-depth so no other ctor
            // invariant can crash the thread either.
            if (std::isnan(score) || score < 0.0 || score > 1.0) {
                static bool warned_bad_score = false;
                if (!warned_bad_score) {
                    warned_bad_score = true;
                    GST_WARNING("hailocachereader: dropping cached detection "
                                "with out-of-range score=%.9g (valid range "
                                "[0,1]); cache row may be corrupted or "
                                "foreign. Further such warnings suppressed.",
                                score);
                }
            } else {
                try {
                    auto det = std::make_shared<HailoDetection>(
                        HailoBBox((float)x, (float)y, (float)w, (float)h),
                        (int)cls, /*label=*/std::string(), (float)score);
                    roi->add_object(det);
                } catch (const std::exception& ex) {
                    static bool warned_ctor = false;
                    if (!warned_ctor) {
                        warned_ctor = true;
                        GST_WARNING("hailocachereader: dropping cached "
                                    "detection that failed to construct: %s. "
...
#if defined(HAVE_GSTHAILOMETA)
            // Restore the cached detections into the buffer's HailoROI so a
            // downstream tap / hailocachebypass sees the same per-tile
            // detection stream the live pass produced (bit-exact gate).
            restore_cached_dets_to_roi(buf, accumulated_dets);
#endif
        }
        return GST_FLOW_OK;
    }
