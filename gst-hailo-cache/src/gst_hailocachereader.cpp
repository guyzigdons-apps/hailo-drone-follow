...
// with tile-local normalized coords. We parse it with a tiny field scanner
// (no JSON lib dependency) keyed by the `"<key>":` tokens, so field order
// is irrelevant and trailing precision is preserved exactly (strtod on the
// %.9g text the writer wrote).
static bool
...
restore_cached_dets_to_roi(GstBuffer* buf, const std::string& dets_json)
{
    ...
    while (i < n) {
        ...
        bool ok = extract_number_field(obj, "cls", &cls)
                  && extract_number_field(obj, "score", &score)
                  && extract_number_field(obj, "x", &x)
                  && extract_number_field(obj, "y", &y)
                  && extract_number_field(obj, "w", &w)
                  && extract_number_field(obj, "h", &h);
        if (ok) {
