...
- `--record` — Pure-GStreamer recording branch (videoconvert -> x264enc -> matroskamux -> filesink). Output: `robot_follow/recordings/rec_<timestamp>.mkv` (override with `--record-output`). Auto-starts after pipeline is PLAYING; can be toggled mid-flight from the web UI / OpenHD. One file per pipeline run — pause/resume creates a time gap inside the same file.
