...
- [ ] **RENAME-01**: Package directory `drone_follow/` renamed to `robot_follow/`; all internal imports updated to `from robot_follow.*`.
- [ ] **RENAME-02**: `pyproject.toml` package name is `robot_follow`; console scripts: `robot-follow` (primary) and `drone-follow` (alias pointing at same `main()`).
...
- [ ] **ABS-01**: `robot_api/robot.py` defines `Robot` protocol (`connect`, `start_session`, `send_command`, `send_zero`, `shutdown`) and `Capabilities` named tuple (`has_altitude`, `needs_offboard_handshake`, `needs_takeoff_landing`, `yaw_units: Literal["deg/s", "rad/s"]`).
- [ ] **ABS-02**: `RobotCommand(forward, yaw, altitude=0.0)` replaces `VelocityCommand` (same three-field shape; drone adapter unchanged at the wire, rover adapter ignores `altitude` and converts deg/s yaw to rad/s).
...
- [ ] **ABS-04**: `live_control_loop` gates the altitude-hold P-loop on `capabilities.has_altitude` (rover sees no `down` term emitted from controller).
- [ ] **ABS-05**: `live_control_loop` gates the bottom-edge "retreat from tilt" safety on `capabilities.has_altitude` (rover gets a different safety strategy in RINT-02).
- [ ] **ABS-06**: `live_control_loop` gates search-after-loss yaw-spin recovery on a capability flag (rovers may not spin to re-find a person; deferred to `RoverCapabilities`).
- [ ] **ABS-07**: `ControllerConfig` altitude fields (`min_altitude`, `max_altitude`, `target_altitude`, `kp_alt_hold`, `max_climb_speed`, `max_down_speed`, `max_bbox_height_safety`, `top_margin_safety`, `bottom_margin_safety`) become `Optional[float]`; `validate()` skips altitude relationship checks when `has_altitude=False`.
...
- [ ] **ROVER-07**: `RoverCapabilities = Capabilities(has_altitude=False, needs_offboard_handshake=False, needs_takeoff_landing=False, yaw_units="rad/s")` — adapter publishes immediately once the rclpy node is up; no offboard / arm / takeoff lifecycle.
...
- [ ] **RINT-02**: Bottom-edge frame safety (currently "person too close → retreat for drone tilt") repurposed for rovers: "person too low in frame → slow/stop". Selected by `capabilities.has_altitude=False`. Drone behaviour at the same call site unchanged.
